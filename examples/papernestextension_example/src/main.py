...
def main(page: ft.Page) -> None:
    ...
    page.add(
        ft.Column(
            spacing=24,
            controls=[
                PageHeader(title="PaperNestExtension", subtitle="Exemples de tous les contrôles disponibles"),
                ft.Row(
                    vertical_alignment=ft.CrossAxisAlignment.START,
                    spacing=24,
                    controls=[PaperNestTextFieldPanel(page), PaperNestDropdownPanel(page)]
                ),
                ft.Row(
                    spacing=24,
                    vertical_alignment=ft.CrossAxisAlignment.START,
                    controls=[
                        ft.Column(spacing=24, expand=1, controls=[PaperNestDatePickerPanel(page), PaperNestColorPickerPanel(page)]),
