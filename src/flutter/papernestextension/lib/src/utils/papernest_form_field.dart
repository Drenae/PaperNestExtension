...
InputBorder? _withPaperNestStateBorder(
  InputBorder? border,
  Color? stateColor,
  double width,
) {
  if (border == null || stateColor == null) {
    return border;
  }
...
InputBorder? _resolvePaperNestFocusedBorder(
  ...
  return baseBorder.copyWith(
    borderSide: BorderSide(
      ...
      width: control.getDouble("focused_border_width", 2.0)!,
    ),
  );
}

InputDecoration buildPaperNestInputDecoration(
  BuildContext context,
  Control control, {
  Widget? customSuffix,
  int? valueLength,
  int? maxLength,
  bool focused = false,
  String fieldState = "normal",
  String? stateMessage,
  Color? stateColor,
  double stateBorderWidth = 1.0,
}) {
  ...
  final resolvedFocusedBorder =
      _resolvePaperNestFocusedBorder(context, control, decoration);

  if (fieldState == "error") {
    decoration = decoration.copyWith(
      errorText: stateMessage,
      enabledBorder: _withPaperNestStateBorder(
        decoration.enabledBorder,
        stateColor,
        stateBorderWidth,
      ),
      errorBorder: _withPaperNestStateBorder(
        decoration.errorBorder ?? decoration.enabledBorder,
        stateColor,
        stateBorderWidth,
      ),
      focusedBorder: resolvedFocusedBorder,
      focusedErrorBorder: resolvedFocusedBorder,
    );
